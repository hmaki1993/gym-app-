...
export function WorkoutSession({ tracker, onClose, onSaved }: Props) {
  ...
  // Initialize base time ΓÇö only on FIRST entry to logging, not on resume
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      // If inside the muscle selector or any allowed scroller ΓåÆ never block
      ...
      // While exercise overlay is open ΓåÆ block horizontal swipes for card navigation
          ...
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '12px', marginTop: '4px' }}>
              ...
              >
                ...
                {hasStartedSession 
                  ? (lang === 'ar' ? '╪º╪│╪¬┘â┘à╪º┘ä ╪º┘ä╪¬┘à╪▒┘è┘å' : 'RESUME SESSION')
                  : (Object.keys(loggedData).length > 0 
                    ? (lang === 'ar' ? '╪º╪│╪¬┘â┘à╪º┘ä ╪º┘ä╪¬┘à╪▒┘è┘å' : 'RESUME WORKOUT') 
