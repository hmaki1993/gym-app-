    ...
    <div style={{ 
      ...
      >
        <div style={{ fontSize: '11px', fontWeight: '950', color: 'var(--text-primary)', opacity: 0.8, textTransform: 'uppercase' }}>
          {dateStr === today ? (lang === 'ar' ? '╪º┘ä┘è┘ê┘à' : 'TODAY') : displayDate}
          <span style={{ marginLeft: '8px', opacity: 0.4 }}>({prs.length})</span>
        </div>
        <div style={{ transform: isDateOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.3s ease', fontSize: '10px', color: 'var(--accent-color)' }}>Γû╝</div>
      </button>
      {isDateOpen && (
        <div style={{ padding: '0 16px 12px', display: 'flex', flexDirection: 'column', gap: '0' }}>
          {(() => {
            ...
            return sortedEntries.map(([mgKey, items], groupIdx) => {
              ...
              const groupName = mg ? (lang === 'ar' ? mg.ar : mg.en) : (lang === 'ar' ? '╪ú╪«╪▒┘ë' : 'OTHER');
                ...
                <React.Fragment key={mgKey}>
                  ...
                  {items.map((pr, idx) => (
                    <div key={pr.exerciseName} style={{ 
                      ...
                      <span style={{ fontSize: '15px', fontWeight: '950', color: 'var(--accent-color)', fontFamily: 'Outfit, sans-serif' }}>{pr.weight} {t(unit as any)} ├ù {pr.reps}</span>
                    </div>
                  ))}
                </React.Fragment>
              );
    ...
    <div ref={containerRef} style={{ display: 'flex', flexDirection: 'column', padding: '10px 4px' }}>
      ...
      <div style={{ position: 'relative' }}>
        {selectedDay && (
          ...
          >
            {lang === 'ar' ? '╪╣╪▒╪╢ ╪º┘ä┘â┘ä Γ£ò' : 'SHOW ALL Γ£ò'}
          </button>
        )}
        <div style={{ 
          display: 'flex', 
          ...
          {[
            { label: selectedDay ? (lang === 'ar' ? '╪¬┘à╪º╪▒┘è┘å ╪º┘ä┘è┘ê┘à' : 'DAY LOGS') : t('thisWeek'), value: selectedDay ? totalWorkouts : weeklyCount, sub: t('workouts'), icon: '≡ƒôà' },
            { label: selectedDay ? (lang === 'ar' ? '╪¬╪º╪▒┘è╪« ╪º┘ä┘è┘ê┘à' : 'LOG DATE') : t('allTime'), value: selectedDay ? new Date(selectedDay).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-GB', { day: 'numeric', month: 'short' }) : tracker.logs.length, sub: t('workouts'), icon: '≡ƒÅå' },
            { label: t('totalVolume'), value: totalVolume > 1000 ? `${(totalVolume / 1000).toFixed(1)}T` : `${totalVolume.toFixed(0)}`, sub: t(unit as any), icon: '≡ƒôê' },
          ].map((card, index) => (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <div>
                        <div style={{ 
                          ...
                          {hasWinsToday ? (lang === 'ar' ? '╪Ñ┘å╪¼╪º╪▓╪º╪¬ ╪º┘ä┘è┘ê┘à! ≡ƒöÑ' : "TODAY'S WINS! ≡ƒöÑ") : t('personalRecord')}
                        </div>
                        {!hasWinsToday && <div style={{ fontSize: '9px', color: 'var(--text-secondary)', fontWeight: '800', marginTop: '2px' }}>{lang === 'ar' ? '╪º╪╢╪║╪╖ ┘ä╪╣╪▒╪╢ ╪º┘ä╪ú╪▒┘é╪º┘à ╪º┘ä┘é┘è╪º╪│┘è╪⌐' : 'TAP TO VIEW ALL RECORDS'}</div>}
                      </div>
                    </div>
                    <div style={{ 
                ...
                {isMasterOpen && (
                  <div style={{ 
                    ...
                    <div style={{ fontSize: '9px', fontWeight: '950', color: 'var(--text-secondary)', letterSpacing: '1px', marginBottom: '5px', paddingLeft: '10px' }}>
                      {lang === 'ar' ? '╪│╪¼┘ä ╪º┘ä╪Ñ┘å╪¼╪º╪▓╪º╪¬ ╪º┘ä╪▓┘à┘å┘è' : 'CHRONOLOGICAL RECORDS'}
                    </div>
                ...
                if (charts.length === 0) {
                  return (
                    <div style={{ 
                      ...
                      <div style={{ fontSize: '24px', marginBottom: '10px' }}>≡ƒôê</div>
                      <div style={{ fontSize: '13px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>
                        {lang === 'ar' ? '┘à╪¡╪¬╪º╪¼ ╪¬╪¬┘à╪▒┘å ╪ú┘â╪¬╪▒!' : 'More Workouts Needed!'}
                      </div>
                      <div style={{ fontSize: '11px', color: 'var(--text-secondary)', lineHeight: '1.4' }}>
                        {lang === 'ar' 
                          ? '╪│╪¼┘ä ╪¬┘à╪▒┘è┘å╪¬┘è┘å ╪╣┘ä┘ë ╪º┘ä╪ú┘é┘ä ┘ä┘å┘ü╪│ ╪º┘ä╪╣╪╢┘ä╪⌐ ╪╣╪┤╪º┘å ┘å┘é╪»╪▒ ┘å┘ê╪▒┘è┘â┘à ╪▒╪│┘à ╪¿┘è╪º┘å┘è ┘ä╪¬╪╖┘ê╪▒ ┘à╪│╪¬┘ê╪º┘â.' 
