      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
        ...
        {pts.map((p, i) => (
          <React.Fragment key={i}>
            <circle cx={p.x} cy={p.y} r={i === pts.length - 1 ? "5" : "2.5"} fill={i === pts.length - 1 ? color : 'var(--text-primary)'} stroke={i === pts.length - 1 ? 'var(--primary-bg)' : 'none'} strokeWidth={i === pts.length - 1 ? "2" : "0"} style={i === pts.length - 1 ? { filter: `drop-shadow(0 0 8px ${color})` } : { opacity: 0.5 }} />
            {(i === 0 || i === pts.length - 1) && (
              <text x={p.x} y={p.y - 12} fill={i === pts.length - 1 ? color : "var(--text-secondary)"} fontSize={i === pts.length - 1 ? "11" : "9"} fontWeight="900" textAnchor={i === 0 ? "start" : (i === pts.length - 1 ? "end" : "middle")} style={i === pts.length - 1 ? { filter: `drop-shadow(0 0 4px rgba(0,0,0,0.5))` } : {}}>{p.val}</text>
            )}
          </React.Fragment>
        ))}
      </svg>
          ...
          {weekDays.map((day, i) => {
            ...
            return (
              <div key={day} onClick={() => setSelectedDay(isSelected ? null : day)} role="button" className="day-column" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: '10px', cursor: 'pointer', paddingBottom: '5px', height: '100%', transition: 'transform 0.2s ease', pointerEvents: 'auto' }}>
                <div style={{ width: '16px', height: `${height}px`, background: isSelected ? `linear-gradient(180deg, var(--text-primary), var(--accent-color))` : (count > 0 ? `linear-gradient(180deg, var(--accent-color), var(--danger-color))` : (isToday ? 'rgba(255, 140, 0, 0.5)' : 'rgba(var(--theme-rgb), 0.3)')), borderRadius: '12px', boxShadow: isSelected ? `0 0 20px var(--accent-color)` : (isToday ? '0 0 10px rgba(255, 140, 0, 0.4)' : 'none'), border: isSelected ? '2px solid var(--text-primary)' : (isToday ? '1px solid rgba(255, 140, 0, 0.6)' : 'none'), transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', opacity: (selectedDay && !isSelected) ? 0.3 : 1, animation: isToday && !isSelected ? 'pulse-orange 2s infinite ease-in-out' : 'none' }} />
        ...
        <div style={{ padding: '24px 0' }}>
          ...
          <div className="hide-scroll" style={{ display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '16px', marginBottom: '8px' }}>
            {MUSCLE_GROUPS.filter(m => loggedMuscles.includes(m.key)).map(m => {
              ...
              return (
                <button key={m.key} onClick={() => setSelectedMuscle(m.key)} style={{ flexShrink: 0, padding: '4px 12px', borderRadius: '6px', background: 'transparent', border: '1px solid', borderColor: isActive ? 'var(--accent-color)' : 'rgba(var(--theme-rgb), 0.05)', color: isActive ? 'var(--accent-color)' : 'var(--text-secondary)', fontWeight: '800', fontSize: '13px', letterSpacing: '0.5px', cursor: 'pointer', transition: 'all 0.2s ease', boxShadow: isActive ? '0 2px 8px rgba(255,140,0,0.15)' : 'none', textTransform: 'uppercase' }}>{lang === 'ar' ? m.ar : m.en}</button>
              );
            })}
          </div>
