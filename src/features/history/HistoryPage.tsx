      ...
      <div style={{ 
              ...
              return (
                <div style={{ 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    <span style={{ fontSize: '15px', fontWeight: '950', color: 'var(--text-primary)', fontFamily: "var(--heading-font)" }}>{monthWorkouts}</span>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    <span style={{ fontSize: '15px', fontWeight: '950', color: 'var(--text-primary)', fontFamily: "var(--heading-font)" }}>{monthRest}</span>
                  </div>
                </div>
              );
            })()}
        ...
        <div style={{ 
          ...
          {(() => {
            ...
            // 1. Initial Padding Cells (Days from Previous Month)
            if (satStartOffset > 0) {
              const prevMonthTotalDays = new Date(currentYear, currentMonth, 0).getDate();
              for (let i = 0; i < satStartOffset; i++) {
                const prevDay = prevMonthTotalDays - satStartOffset + 1 + i;
                dayElements.push(
                  <div 
                    key={`prev-${i}`} 
                    style={{ 
                      background: tracker.settings.themeMode === 'dark' ? '#111114' : '#f5f5f5',
                      width: '100%',
                      aspectRatio: '1',
                      padding: '4px 2px',
                      borderRadius: '8px',
            ...
            for (let day = 1; day <= totalDays; day++) {
              ...
              dayElements.push(
                ...
                >
                  ...
                  {dayMuscles.length > 0 ? (
                    <>
                      {/* Icons: when selected with multiple muscles → horizontal pill overlay */}
                      {active && dayMuscles.length > 1 ? (
                        <div 
                          className="premium-overlay-animate"
                          style={{
                            position: 'absolute',
                            top: '50%',
                            left: '50%',
                            ...
                            boxShadow: tracker.settings.themeMode === 'dark'
                              ? '0 8px 32px rgba(0,0,0,0.45), 0 0 10px rgba(46, 204, 113, 0.2)'
                              : '0 8px 32px rgba(31,38,135,0.15), 0 0 10px rgba(46, 204, 113, 0.15)',
                            whiteSpace: 'nowrap'
                          }}
                        >
                          {/* Caret pointing up to the middle of the cell */}
                          <div style={{
                            position: 'absolute',
                            top: '-5px',
                            left: '50%',
                          ...
                          {dayMuscles.map((g, _idx) => {
                            const mg = MUSCLE_GROUPS.find(m => m.key === g);
                            const iconSize = dayMuscles.length <= 2 ? 26 : 22;
                            return mg?.icon ? (
                              <TransparentImage key={g} src={mg.icon} alt="" width={iconSize} height={iconSize} threshold={45} style={{ filter: 'none', transform: 'scale(1)', transition: 'transform 0.2s' }} />
                            ) : <span key={g} style={{ fontSize: '10px' }}>•</span>;
                          })}
                        </div>
                      ) : (
                        /* Single icon (not selected, or selected with only 1 muscle) */
                        <div style={{ 
                          marginTop: '12px', 
                          display: 'flex', 
                          flexDirection: 'row',
                          ...
                          {(() => {
                            const mg = MUSCLE_GROUPS.find(m => m.key === dayMuscles[0]);
                            return (
                              <>
                                {mg?.icon ? (
                                  <TransparentImage src={mg.icon} alt="" width={22} height={22} threshold={45} style={{ filter: 'none', zIndex: 10 }} />
                                ) : (
                    ...
                    isPast && !isToday && (
                      <div style={{ marginTop: '10px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.6, zIndex: 2 }}>
                        <div style={{
                          width: '22px',
                          height: '22px',
                ...
                return (
                  <div key={idx} style={{ flex: '1', display: 'flex', alignItems: 'center' }}>
                    <div style={{ 
                      ...
                      <span style={{ fontSize: '20px', fontWeight: '950', color: isThisWeek ? 'var(--accent-color)' : (week.count > 0 ? 'var(--text-primary)' : (isLight ? 'rgba(18, 18, 18, 0.25)' : 'rgba(150,150,150,0.3)')), fontFamily: "var(--heading-font)", lineHeight: 1 }}>{week.count}</span>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                  ...
                  <span style={{ fontSize: '11px', fontWeight: '950', color: 'var(--accent-secondary)', textTransform: 'uppercase', letterSpacing: '2px', fontFamily: "var(--heading-font)" }}>{t('session')}</span>
                </div>
                <div style={{ 
                        ...
                        {[
                          ...
                          <React.Fragment key={stat.label}>
                            ...
                            <div style={{ flex: 1, textAlign: 'center' }}>
                              <div style={{ fontSize: '20px', fontWeight: '950', color: tracker.settings.themeMode === 'dark' ? '#fff' : 'var(--text-primary)', fontFamily: "var(--heading-font)" }}>{stat.value}</div>
                          ...
                          return sortedExercises.map((ex, exIdx) => {
                            ...
                            return (
                              <React.Fragment key={`${ex.name}-${exIdx}`}>
                                ...
                                <div style={{ opacity: expandedLogId === log.id ? 1 : 0, transform: expandedLogId === log.id ? 'translateX(0)' : 'translateX(-10px)', transition: `all 0.4s ease ${0.1 + exIdx * 0.05}s`, background: 'rgba(var(--theme-rgb), 0.06)', borderRadius: '14px', borderLeft: ex.isFirstInGroup ? '3px solid var(--accent-color)' : '3px solid rgba(var(--theme-rgb), 0.15)', overflow: 'hidden' }}>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                      <div style={{ textAlign: 'right' }}>
                                        <span style={{ fontSize: '14px', fontWeight: '950', color: tracker.settings.themeMode === 'dark' ? '#fff' : 'var(--text-primary)' }}>{ex.sets.length}</span>
                                        <span style={{ fontSize: '9px', fontWeight: '950', color: isLight ? 'var(--text-primary)' : 'var(--text-secondary)', marginLeft: '3px' }}>{t('sets').toUpperCase()}</span>
                                      </div>
                                    </div>
                                  </div>
                                  ...
                                  <div style={{
                                    ...
                                    {ex.sets.map((set, setIdx) => {
                                      ...
                                      const convertedWeight = tracker.convertWeight(set.weight, (set as any).unit || 'kg', displayUnit);
                                      const roundedWeight = Number(convertedWeight.toFixed(1));
                                      const isThisBest = set === bestSet || (convertedWeight === tracker.convertWeight(bestSet.weight, (bestSet as any).unit || 'kg', displayUnit));
                                      return (
                                        <div key={setIdx} style={{
                                          ...
                                          <span style={{ fontSize: '8px', fontWeight: '800', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                            {lang === 'ar' ? `${setIdx + 1}` : `S${setIdx + 1}`}
                                          </span>
                                          <span style={{ fontSize: '13px', fontWeight: '950', color: isThisBest ? 'var(--accent-color)' : (tracker.settings.themeMode === 'dark' ? '#fff' : 'var(--text-primary)'), fontFamily: "var(--heading-font)", lineHeight: 1 }}>
                                            {roundedWeight}<span style={{ fontSize: '8px', fontWeight: '800', marginLeft: '1px' }}>{t(displayUnit as any)}</span>
                                          </span>
                                          <span style={{ fontSize: '10px', fontWeight: '950', color: 'var(--text-secondary)', marginTop: '1px' }}>
                                            × {set.reps} <span style={{ fontSize: '8px' }}>{t('reps')}</span>
                                          </span>
                                        </div>
                                      );
                                    })}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', gap: '4px' }}>
                              {[
                                ...
                                <React.Fragment key={stat.label}>
                                  ...
                                  <div style={{ flex: 1, textAlign: 'center' }}>
                                    <div style={{ fontSize: '15px', fontWeight: '950', color: idx === 0 ? 'var(--accent-color)' : (tracker.settings.themeMode === 'dark' ? '#fff' : 'var(--text-primary)'), fontFamily: "var(--body-font)" }}>{stat.value}</div>
                                    <div style={{ fontSize: '10px', color: isLight ? 'rgba(var(--theme-rgb), 0.75)' : 'rgba(var(--theme-rgb), 0.5)', fontWeight: '950', letterSpacing: '2px', marginTop: '6px', textTransform: 'uppercase' }}>{stat.label}</div>
                                  </div>
                                </React.Fragment>
                              ))}
                              ...
                              {dayNutrition.map((food: any, fIdx: number) => (
                                <div key={fIdx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(var(--theme-rgb), 0.1)', padding: '8px 12px', borderRadius: '10px' }}>
                                  ...
                                  <div style={{ textAlign: 'right', minWidth: '60px' }}>
                                    <span style={{ fontFamily: "var(--body-font)", fontSize: '13px', fontWeight: '800', color: 'var(--accent-color)' }}>{food.calories}</span>
