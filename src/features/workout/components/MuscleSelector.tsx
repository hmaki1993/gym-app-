        ...
        {MUSCLE_GROUPS.map(mg => {
          ...
          return (
            <button key={mg.key} onClick={() => onSelect(mg.key)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, background: 'none', border: 'none', padding: 0, flexShrink: 0, minWidth: 60, cursor: 'pointer', transition: 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)', opacity: isActive ? 1 : hasExercises ? 0.9 : 0.5, position: 'relative' }}>
              <div style={{ width: 75, height: 75, borderRadius: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isActive ? 'rgba(var(--theme-rgb), 0.08)' : 'rgba(var(--theme-rgb), 0.04)', border: `2px solid ${isActive ? 'var(--accent-color)' : 'rgba(var(--theme-rgb), 0.08)'}`, boxShadow: isActive ? '0 0 25px -4px var(--accent-color-alpha)' : 'none', transition: 'all 0.4s cubic-bezier(0.2, 0.8, 0.2, 1)', transform: isActive ? 'translateY(-2px)' : 'none', overflow: 'hidden', position: 'relative' }}>
